...
| 5.7 | JWT access-token infrastructure — `AccessTokenKeyMaterial` (Ed25519 JWK parsing + JCA signer/verifier), `AccessTokenIssuer` (EdDSA-signed JWT with iss/aud/sub/sid/jti/iat/exp, kid header, 15m TTL), `AccessTokenVerifier`, `AccessTokenConfig`; `IssuedSession` extended with `accessToken`; `SessionLifecycleService` wired to issue access tokens on session create/refresh. Per [ADR-0016](../docs/architecture/decisions/0016-access-token-design.md). | **PUSHED** | Branch `feat/identity-access-token-infrastructure` — PR not yet created (`gh` CLI not authenticated) |
| 5.8 | `ResolveAuthenticatedPrincipal` public application API — verifies JWT signature/claims then validates server-side session state (ACTIVE, not expired, identity match); revoked session invalidates an otherwise valid JWT. `AuthenticatedPrincipal` public DTO. Public API surface test updated. | **PUSHED** | Same branch as 5.7 |

## Phase 5 status

As of checkpoint 5.8, every `identity` public application API member that can be built **without the `accounts` module** is complete and tested: `Authenticate`, `RefreshSession`, `RevokeSession`, `RevokeAllUserSessions`, `ListUserSessions`, `ResolveAuthenticatedPrincipal`. The access-token design decision was recorded as [ADR-0016](../docs/architecture/decisions/0016-access-token-design.md) (JWT/JWS, EdDSA/Ed25519, 15-minute TTL, mandatory server-side session validation).

One public API member remains unimplemented:
...
No HTTP endpoints exist yet for any of `identity`'s API — that is a separate, later subtask category.

**Recommended next step:** create the PR for branch `feat/identity-access-token-infrastructure` (requires `gh auth login`), merge to main, then either start Phase 6 (`accounts`) or implement `identity` HTTP endpoints for the already-complete public API.
