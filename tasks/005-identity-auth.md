...
| 5.3 | Next: select the smallest bounded next subtask from current repo state (e.g. `AuthDevice`/`AuthSession` lifecycle helpers, or a minimal `accounts` persistence primitive to unblock `ProvisionUser`) | **NEXT** | — |
