...
| `CompanyMemberRemoved` | `accounts` | membership inactive bo‘ldi | `eventId`, `companyId`, `memberId`, `userId`, `actorUserId` | `communication`; accounts ichki permission-cache invalidation | reliable async notification; authorizationning source of truthi har requestda accounts, event cache correctnessga yagona tayanch emas |
...
| `AvailableVehicleClosed` | `fleet` | availability closed/expired bo‘ldi | `eventId`, `availableVehicleId`, `vehicleId`, `reasonCode`, `closedAt` | `marketplace` matching reaction if enabled | reliable async; listing/version dedupe |
